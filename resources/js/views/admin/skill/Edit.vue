  ...
  <div>
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
      <div class="px-4 py-5 sm:px-6">
        ...
        <p class="mt-1 max-w-2xl text-sm text-gray-500">Update the details for the skill</p>
      </div>
      <div class="border-t border-gray-200">
        <form @submit.prevent="updateSkill" class="p-6">
...
const error = ref('');
const skillId = ref(parseInt(route.params.id)); // Convert to integer
...
const loadSkill = async () => {
  try {
    const skill = await careerStore.getSkill(skillId.value);
    if (skill) {
...
const updateSkill = async () => {
  ...
  try {
    await careerStore.updateSkill(skillId.value, skillForm);
    router.push('/admin/skills');
  } catch (err) {
    ...
    if (err.response?.data?.message) {
      ...
      error.value = 'An error occurred while updating the skill. Please try again.';
    }
  } finally {
    loading.value = false;
  }
