<template>
  <div>
    <h1 class="text-2xl font-bold text-gray-900 mb-6">Add New Internship</h1>
    
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
      <div class="px-4 py-5 sm:px-6">
        <h2 class="text-lg leading-6 font-medium text-gray-900">Internship Information</h2>
        <p class="mt-1 max-w-2xl text-sm text-gray-500">Fill in the details for the new internship</p>
      </div>
      <div class="border-t border-gray-200">
        <form @submit.prevent="createInternship" class="p-6">
...
const createInternship = async () => {
  ...
  try {
    const response = await useApi().post('/internships', internshipForm);
    ...
    if (err.response?.data?.message) {
      ...
      error.value = 'An error occurred while creating the internship. Please try again.';
    }
  } finally {
    loading.value = false;
  }
