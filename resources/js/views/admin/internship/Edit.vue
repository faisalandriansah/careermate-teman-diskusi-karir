  ...
  <div class="space-y-6">
    <div class="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-6 shadow-[0_8px_30px_rgba(15,23,42,0.04)] sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        <h2 class="mt-1 text-2xl font-semibold text-slate-900">Edit Internship</h2>
        <p class="mt-2 text-sm text-slate-500">Perbarui informasi magang dengan desain yang lebih bersih.</p>
      </div>
      <button
        @click="router.go(-1)"
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-[0_8px_30px_rgba(15,23,42,0.04)]">
      <form @submit.prevent="handleSubmit" class="space-y-5">
        ...
        <div>
          <label class="mb-2 block text-sm font-medium text-slate-700">Duration (months)</label>
          <input
            type="number"
            v-model="internship.duration"
...
const internship = ref({
  ...
  location: 'Jakarta',
  duration: 6,
  description: 'Learn software development in a real-world environment'
});

const handleSubmit = () => {
  console.log('Updating internship:', internship.value);
