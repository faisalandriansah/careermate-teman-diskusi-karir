        ...
        >
            <div>
                ...
                <p class="mt-2 text-sm text-slate-500">
                    Perbarui informasi magang dengan desain yang lebih bersih.
                </p>
            </div>
            <button
                @click="router.go(-1)"
...
onMounted(async () => {
    try {
        ...
        const [internshipResponse, careerResponse] = await Promise.all([
            getInternship(internshipId),
            getCareers(),
        ]);

        internship.value = internshipResponse.data;

        careers.value = careerResponse.data.data;
    } catch (error) {
        console.error(error);
    }
